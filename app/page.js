...
export default function App() {
  ...
  // Mobile App Download Banner State
  ...
  // Handle direct APK download or PWA installation
  ...
  if (!isLoggedIn) {
    return (
      <div className="h-[100dvh] w-screen fixed inset-0 flex items-center justify-center bg-[#070b14] text-white p-4 font-sans overflow-hidden">
        {toastMessage && (
          <div className="fixed top-4 right-4 z-50 bg-slate-900 text-white px-4 py-3 rounded-2xl shadow-2xl border border-amber-500/40 flex items-center gap-2 text-xs animate-bounce">
  ...
  return (
    <div className={`h-[100dvh] w-screen fixed inset-0 overflow-x-hidden overflow-y-hidden flex flex-col font-sans transition-colors duration-300 ${theme === 'dark' ? 'bg-[#070b14] text-slate-100' : 'bg-slate-100 text-slate-800'}`}>
      ...
      <header className={`h-16 border-b px-4 flex items-center justify-between backdrop-blur-md z-20 flex-shrink-0 ${theme === 'dark' ? 'border-slate-800/80 bg-[#0a0f1d]/90' : 'border-slate-200 bg-white/90'}`}>
        <div className="flex items-center space-x-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[11px] text-emerald-400 flex items-center gap-1 font-medium truncate">
              <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse flex-shrink-0"></span> @{currentUser.username}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-2 flex-shrink-0">
          <button
            ...
            className="px-2.5 sm:px-3 py-1.5 bg-amber-500/20 text-amber-400 hover:bg-amber-500 hover:text-black border border-amber-500/40 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-all"
            title="Download App"
          >
            <Download className="w-3.5 h-3.5 flex-shrink-0" /> <span className="hidden sm:inline">Download App</span>
          </button>

          <button
            onClick={() => setShowRequestsModal(true)}
            className="relative w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-slate-800/60 flex items-center justify-center text-slate-300 hover:bg-slate-700/60 transition-all"
            title="Friend Requests"
          >
            <Bell className="w-4 h-4 sm:w-5 sm:h-5 text-amber-400" />
            {incomingRequests.length > 0 && (
              <span className="absolute -top-1 -right-1 w-4 h-4 sm:w-5 sm:h-5 bg-red-500 text-white rounded-full text-[10px] font-bold flex items-center justify-center animate-pulse">
          ...
          <button
            ...
            className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-slate-800/60 flex items-center justify-center text-slate-300 hover:bg-slate-700/60 transition-all"
          >
          ...
          <div
            ...
            className="flex items-center space-x-1.5 cursor-pointer bg-slate-800/40 p-1 pr-2 sm:pr-3 rounded-full border border-slate-700/50 hover:bg-slate-700/40 transition-all"
          >
            <img src={currentUser.avatar} className="w-7 h-7 sm:w-8 sm:h-8 rounded-full object-cover" alt="" />
            <span className="text-xs font-semibold max-w-[80px] truncate hidden sm:inline-block">
              @{currentUser.username}
            </span>
          </div>

          <button onClick={handleLogout} className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-red-500/10 text-red-400 flex items-center justify-center border border-red-500/20 hover:bg-red-500/20 transition-all">
      ...
      <div className="flex-1 flex overflow-hidden min-h-0 relative w-full max-w-full">
        ...
        <aside className={`w-full sm:w-80 md:w-96 border-r flex flex-col z-10 ${mobileChatOpen ? 'hidden sm:flex' : 'flex'} ${theme === 'dark' ? 'bg-[#0b101e]/80 border-slate-800/80' : 'bg-white border-slate-200'}`}>
          ...
          <div className={`flex items-center border-b p-1.5 sm:p-2 gap-1 flex-shrink-0 ${theme === 'dark' ? 'border-slate-800/80 bg-[#070b15]' : 'border-slate-200 bg-slate-50'}`}>
            <button onClick={() => setActiveTab('chats')} className={`flex-1 py-2 rounded-xl text-xs font-semibold flex items-center justify-center gap-1 transition-all ${activeTab === 'chats' ? 'bg-slate-800 text-amber-400' : 'text-slate-400'}`}>
              <MessageSquare className="w-3.5 h-3.5" /> Chats
            </button>
            <button onClick={() => setActiveTab('status')} className={`flex-1 py-2 rounded-xl text-xs font-semibold flex items-center justify-center gap-1 transition-all ${activeTab === 'status' ? 'bg-slate-800 text-amber-400' : 'text-slate-400'}`}>
              <Circle className="w-3.5 h-3.5" /> Status
            </button>
            <button onClick={() => setActiveTab('calls')} className={`flex-1 py-2 rounded-xl text-xs font-semibold flex items-center justify-center gap-1 transition-all ${activeTab === 'calls' ? 'bg-slate-800 text-amber-400' : 'text-slate-400'}`}>
              <Phone className="w-3.5 h-3.5" /> Calls
            </button>
            <button onClick={() => setActiveTab('profile')} className={`flex-1 py-2 rounded-xl text-xs font-semibold flex items-center justify-center gap-1 transition-all ${activeTab === 'profile' ? 'bg-slate-800 text-amber-400' : 'text-slate-400'}`}>
        ...
        <main className={`flex-1 flex flex-col relative h-full min-h-0 w-full max-w-full ${!mobileChatOpen ? 'hidden sm:flex' : 'flex'} ${theme === 'dark' ? 'bg-[#070b15]/90' : 'bg-slate-100'}`}>
          {!activeChat ? (
            ...
            <div className="flex-1 flex flex-col h-full min-h-0 overflow-hidden w-full max-w-full">
              <div className={`h-16 border-b px-4 flex items-center justify-between flex-shrink-0 ${theme === 'dark' ? 'border-slate-800/80 bg-[#0a0f1e]/90' : 'border-slate-200 bg-white'}`}>
                <div className="flex items-center space-x-3 min-w-0">
                  <button onClick={() => setMobileChatOpen(false)} className="sm:hidden p-2 text-slate-400 hover:text-white flex-shrink-0">
                  ...
                  <div className="min-w-0">
                    <h3 className="font-bold text-sm truncate">@{activeChat.username}</h3>
                    <p className={`text-[11px] font-medium truncate ${isPeerActiveNow ? 'text-emerald-400' : 'text-slate-400'}`}>
              ...
              {/* MESSAGES AREA */}
              <div className="flex-1 p-3 sm:p-4 overflow-y-auto space-y-3 min-h-0 w-full" onClick={() => setContextMenuMsg(null)}>
                {messages.length === 0 ? (
                    ...
                    return (
                      ...
                      >
                        <div
                          ...
                          className={`max-w-[85%] sm:max-w-[80%] p-3 rounded-2xl shadow-md relative group/msg ${
                        ...
                        >
                          ...
                          {msg.isDeletedEveryone ? (
                            ...
                            <>
                              {msg.image && <img src={msg.image} className="max-w-xs w-full rounded-xl mb-2 object-cover" alt="" />}
                              
                              {/* AUDIO VOICE NOTE PLAYER */}
                              {msg.audio && (
                                <div className="mb-2 flex flex-col gap-1 bg-black/30 p-2.5 rounded-xl border border-white/10 min-w-[180px] sm:min-w-[240px]">
              ...
              {isRecordingAudio ? (
                <div className="p-3 border-t border-slate-800/80 bg-red-950/40 flex items-center justify-between space-x-3 flex-shrink-0 animate-pulse">
                  <div className="flex items-center space-x-2 min-w-0">
                    ...
                    <span className="text-xs font-semibold text-red-400 truncate">Recording... ({recordingDuration}s)</span>
                  </div>
                  <div className="flex items-center space-x-2 flex-shrink-0">
                ...
                <div className="p-3 border-t border-slate-800/80 bg-slate-900/90 flex items-center space-x-2 flex-shrink-0">
                  <label className="p-2 text-slate-400 hover:text-amber-400 cursor-pointer transition-colors flex-shrink-0">
                    <Paperclip className="w-5 h-5" />
                    <input type="file" accept="image/*" className="hidden" onChange={handleImageAttachment} />
                  </label>

                  <button onClick={startRecordingAudio} className="p-2 text-slate-400 hover:text-red-400 transition-colors flex-shrink-0" title="Hold to record voice note">
                  ...
                  <input
                    ...
                    className="flex-1 bg-slate-800 border border-slate-700 rounded-2xl py-2.5 px-4 text-base sm:text-xs text-white focus:outline-none focus:border-amber-500 min-w-0"
                  />
                  <button onClick={handleSendMessage} className="p-2.5 bg-amber-500 hover:bg-amber-400 text-black font-bold rounded-xl shadow-lg shadow-amber-500/20 transition-colors flex-shrink-0">
